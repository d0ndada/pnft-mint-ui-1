...
export const AboutSection = () => {
  ...
  return (
   <section id="about" className="flex h-[100vh] flex-col items-center justify-center space-y-10  p-5 text-foreground scroll-snap-align-start">
      <h2 ref={ref} className={`mt-[-15%] p-[6%] text-4xl font-bold text-foreground ${inView ? 'animate-fade-in-up' : ''}`}>About Us</h2>
      <div className="flex flex-row">
        <p className={`max-w-[700px] text-left text-foreground ${inView ? 'animate-fade-in-up' : ''}`}>
        ...
        <div className="flex justify-center gap-5">
          <div className={`relative ml-[12%] ${inView ? 'animate-fade-in-up' : ''}`}>
