...
function TeamMember({ image, name, role, description }: TeamMemberProps) {
  return (
      ...
      <div className="flex justify-center items-center bg-[#F4F4F4] w-full h-[250px] hover:shadow-lg transistion-shadow "> 
        <Image className="rounded-full border-4 border-#E0E0E0" width={250} height={250} src={image} alt={name} />
      </div>
      <div className="justify-flex flex w-[100%] p-2 flex-col border-2 items-center bg-[#ffebcd]">
        <h3 className="text-lg font-bold">{name}</h3>
        <p className="text-m text-foreground underline pb-1 decoration-2 decoration-primary">{role}</p>
 ...
 <section id="team" className="flex flex-col items-center space-y-8 px-4 py-12 scroll-snap-align-start">
        ...
        <div className="flex flex-col items-center space-y-8 w-[80%] max-w-6xl"> {/* This is the flex container */}
            <div className=" gap-[7rem] flex w-[80%]">
        ...
        <div ref={ref} className={`flex flex-col items-center  border-2 border-#E0E0E0 bg-[#ffebcd] p-1  md:w-2/3 hover:shadow-md transition-shadow   ${inView ? ' animate-fade-in-up' : ''} `}> {/* Adjusted width for responsiveness */}
